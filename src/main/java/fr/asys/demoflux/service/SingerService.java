...
public class SingerService {
    ...
    private static final List<Singer> SINGERS = List.of(new Singer("Casablancas", "Julian"), new Singer("Kiwanuka", "Michael"), new Singer("Jagger", "Mick"), new Singer("Alt-J", null));
    ...
    public SingerDto getSynchronousSingerFromSingerName(String singerName) {
        ...
        return SINGERS.stream().filter(singer -> singer.name().equals(singerName)).findFirst().map(singer -> new SingerDto(singer.name(), singer.firstname())).orElseThrow(IllegalAccessError::new);
    }
